...
import { goTo, logOut, updateTaxonomies } from 'actions/index';
...
export class CMS extends React.PureComponent {
  ...
  render() {
    const { item, isPortalActive } = this.state;
    ...
    return (
      <div key="CMS" className="app__cms app__route">
        ...
        <div className="app__container">
          {output.toolbar}
          {output.table}
          <Portal isActive={isPortalActive} onClose={this.handleHidePortal}>
            <FormItem
              item={item}
              categories={categories}
              tags={tags}
              hidePortal={this.handleHidePortal}
            />
          </Portal>
        </div>
      </div>
