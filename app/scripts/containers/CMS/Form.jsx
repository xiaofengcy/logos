...
import { updateLogos } from 'actions/index';
...
export class FormItem extends React.PureComponent {
  ...
  handleSubmit = (formData) => {
    const { item, dispatch, reset, hidePortal } = this.props;
  ...
  renderMultiselect({ input, label, data }) {
    ...
    if (name === 'files') {
      tagComponent = val => (
        <div><img src={`${config.cdnURL}${encodeURIComponent(val.item)}`} alt={val.item} /></div>);
    }
      ...
      <form className="app__form" onSubmit={handleSubmit(this.handleSubmit)}>
        ...
        <div className="app__form__content">
          ...
          <div>
            ...
            />
            <div className="row">
              <div className="col-xs-12 col-ix-4">
                <Field
                  name="favorite"
                  label="Favorite"
                  component={this.renderInputSwitch}
